...
class AuthController extends BaseController
{
        ...
        if ($user) {
            ...
            if ($user1) {
                if($request->type == 'forgot'){
                    ...
                    $smessage = "Otp sent to your Email.";
                }else{
                    ...
                    $message = $twilio->messages
                        ...
                        $smessage = "Otp sent to your Mobile number.";
                }
                ...
                return $this->sendResponse($success, $smessage);
            } else {
                return response()->json([
                    'status' => 406,
                    'message' => 'Invalid Otp.',
    ...
    public function forgotpassword(Request $request)
    {
        ...
        $otp = rand(1000, 9999);
        // Log::info("password_otp = ".$otp);
        // $users = unverified_user::where('email', $request->email)->first();
        // if(!$users){
            $user = User::where('email', $request->email)->first();
        // }else{
        //     $user = $users;
    ...
    public function allsociallogoutsapi(Request $request)
    {
        ...
        if ($user) {
            ...
            $createUser = User::updateOrCreate([
                ...
                'social_type' => $request->socialtype,
                'image' => ($request->socialimage != null) ? $request->socialimage : null,
                'password' => encrypt('john123'),
