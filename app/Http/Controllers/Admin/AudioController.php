...
class AudioController extends Controller
{
    ...
    public function audiostore(Request $request)
    {
        ...
        $audiofilename = $audiofile->getClientOriginalName();
            if (!file_exists(public_path('assets/graph/audio/block'.$request->block))) {
                mkdir(public_path('assets/graph/audio/block'.$request->block), 0777, true);
            }
    
            $destinationPath_2 = public_path('assets/graph/audio/block'.$request->block);
    ...
    public function updateaudio(Request $request, $id)
    {
        ...
        if ($audiofile != null) {
            ...
            if (!file_exists($destinationPath_2)) {
                mkdir($destinationPath_2, 0777, true);
            }

            if (file_exists($destinationPath_2 . $audiofilename)) {
                unlink($destinationPath_2 . $audiofilename);
            }
