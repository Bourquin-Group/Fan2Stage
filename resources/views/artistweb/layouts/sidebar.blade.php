<div class="col-lg-3 col-md-12 col-sm-12 col-xs-12 tab-navbar">
    ...
    <div class="tab-list nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
        <h6 class="font-18">Fan2Stage And You</h6>
          ...
          <a href="{{route('profile')}}" class="font-18 nav-tab-btn {{ (request()->is('web/profile') || request()->is('web/editprofile')) ? 'active' : '' }}" style="{{(!auth()->user()->subscription_plan_id  || auth()->user()->verified_profile == 0)? 'pointer-events: none;': '' }}" > 
          ...
          <a href="{{route('subscription')}}" class="font-18 nav-tab-btn {{ 
            (request()->is('web/subscription') || request()->is('web/subscription-payment/*')) ? 'active' : '' }}" style="{{(!auth()->user()->subscription_plan_id  || auth()->user()->verified_profile == 0)? 'pointer-events: none;': '' }}"  > 
         ...
         {{-- <h6 class="font-18">Other</h6> --}}
          <a href="{{route('aboutus')}}" class="font-18 nav-tab-btn {{ (request()->is('web/aboutus')) ? 'active' : '' }}" style="{{(!auth()->user()->subscription_plan_id  || auth()->user()->verified_profile == 0)? 'pointer-events: none;': '' }}" > 
          ...
          <a href="{{route('termscondition')}}" class="font-18 nav-tab-btn {{ (request()->is('web/termscondition')) ? 'active' : '' }}" style="{{(!auth()->user()->subscription_plan_id  || auth()->user()->verified_profile == 0)? 'pointer-events: none;': '' }}"   > 
          ...
          <a href="{{route('privacypolicy')}}" class="font-18 nav-tab-btn {{ (request()->is('web/privacypolicy')) ? 'active' : '' }}" style="{{(!auth()->user()->subscription_plan_id  || auth()->user()->verified_profile == 0)? 'pointer-events: none;': '' }}"  > 
