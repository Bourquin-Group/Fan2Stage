...
<section class="main_section custom_container">
    <div class="navgat_otherpage">
        <h1 class="task_titlt"><a href="{{ url('web/artisthome')}}"><span><img src="{{ asset('assets/web/images/arrow-left.svg') }}" alt="arrow"></span></a> Followers</h1>
    </div>
  <!-- content  -->
  <div class="followers_section">
    @if(count($followerslist) > 0)
    @foreach($followerslist as $f_list )
    ...
    justify-content: center;">
        <h4 class="task_titlt">No Followers For You <span style="font-weight: bold">{{Auth::user()->name}}</span>!!</h4>
    </div>
    @endif
  </div>
</section>
